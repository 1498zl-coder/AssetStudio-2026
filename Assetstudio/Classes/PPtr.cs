        ...
        private bool TryGetAssetsFile(out SerializedFile result)
        {
            ...
            if (m_FileID > 0 && m_FileID - 1 < assetsFile?.m_Externals.Count)
            {
                ...
                if (index == -2)
                {
                    ...
                    {
                        index = assetsFileList?.FindIndex(x => x?.fileName.Equals(name, StringComparison.OrdinalIgnoreCase) ?? false) ?? -1; // 新增：空值校验
                        assetsFileIndexCache.Add(name, index);
                    }
                }

                if (index >= 0 && assetsFileList != null && index < assetsFileList.Count)
                {
