    ...
    public class SerializedSubProgram
    {
        ...
        public SerializedSubProgram(ObjectReader reader)
        {
            ...
            try
            {
                ...
                {
                    Logger.Warning($"着色器子程序数据不足,跳过解析,位置:{startPosition},剩余:{reader.Remaining}");
                    return;
                }

                m_Channels = new ParserBindChannels(reader);

                if ((version[0] >= 2019 && version[0] < 2021) || (version[0] == 2021 && version[1] < 2) || HasGlobalLocalKeywordIndices(reader.serializedType))
                {
                ...
                else
                {
                    ...
                    {
                        m_KeywordIndices = new ushort[0];
                    }
                }

                if (reader.Remaining < 2)
                {
                    Logger.Warning($"着色器子程序数据不足，无法读取硬件层和GPU程序类型。位置: {reader.Position}, 剩余: {reader.Remaining}");
                    return;
                }
                ...
                else
                {
                    if (reader.Remaining <= 0)
                    {
                        Logger.Warning($"着色器子程序无参数数据，跳过解析。位置: {reader.Position}, 剩余: {reader.Remaining}");
                        return;
                    }
                    ...
                    {
                        Logger.Warning($"解析着色器子程序参数时数据不足: {e.Message}");
                        return;
                    }
                }
            ...
            {
                Logger.Warning($"无法解析着色器子程序,数据读取越界,起始位置:{startPosition},当前位置:{reader.Position},剩余:{reader.Remaining},错误:{e.Message}");
            }
            catch (Exception e)
            {
                Logger.Warning($"解析着色器子程序时发生未知错误:{e.Message}");
            }
        }
    }
